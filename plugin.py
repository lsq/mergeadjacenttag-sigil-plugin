...
def run(bk):
    ...
    if bk.launcher_version() < 20150909:
        print(
            "Error: The %s plugin requires Sigil version 0.8.900 or higher."
            % bk._w.plugin_name
        )
        return -1
    ...
    prefs_folder = os.path.join(
        os.path.dirname(bk._w.plugin_dir), "plugins_prefs", bk._w.plugin_name
    )
    ...
    if not bk._w.using_automate or (
        bk._w.using_automate
        and not prefs["miscellaneous_settings"]["automate_runs_headless"]
    ):
    ...
    if bk._w.automate_parameter.strip() == "":
        ...
        headless_prefs = two_up.joinpath(
            "plugins_prefs", bk._w.plugin_name, "headless.json"
        )
        if headless_prefs.exists and headless_prefs.is_file():
            ...
            for ident, href in bk.text_iter():
                ...
                try:
                    ...
                    print("{} {}! {}.\n".format("Error parsing", href, "File skipped"))
                    continue
                ...
                if occurrences:
                    ...
                    print("{} {}\n".format("Criteria not found in", href))
        else:
